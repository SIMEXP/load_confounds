...
def clean_nii(fmri_path, output, scrubbing):
    ...
    fmri_path_out = (
        str(output)
        + "/"
        + fmri_path.split("/")[-1].split(".nii.gz")[0]
        + "_clean.nii.gz"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "preproc_dir", type=str, help="resmaple folder from NIAK output directory"
    )
    parser.add_argument(
        "output_dir", type=str, help="output directory for scrubbed data"
    )
    parser.add_argument(
        "--scrub",
        type=bool,
