...
confounds, sample_mask = Minimal().load(file)

# Use the confounds and sample mask to load preprocessed time series with nilearn
...
```
It is also possible to fine-tune a subset of noise components and their parameters with `Confounds`:
```python
from load_confounds import Confounds
confounds, sample_mask = Confounds(strategy=['high_pass', 'motion', 'global'], motion="full").load(file)
```
...
## Flexible confound strategey definition

`load_confounds` provides a flexible API to define confound regressors from fMRIPrep outputs.
The regressors are grouped into noise compoenents to ensure sensible confound regressor selection.
...
In nilearn 0.8.0, `sample_mask` is introduced as a new parameter of the `transform` method in `NiftiMasker`.
When outlier volumes are present, `sample_mask` has a shape of number of scans - number of volumes removed and masks the niimgs along time/fourth dimension to perform scrubbing (remove volumes with high motion) and/or non-steady-state volumes. `load_confounds` always detects the presents of non-steady-state volumesin the confound files.
Otherwise `sample_mask` would be `None`.


## Predefined strategies

The package supplies several predefined strategies based on past literatures to ensure correct confound regressors retrieval.
...
Note that if a `.nii.gz` file is specified, `load_confounds` will automatically look for the companion `tsv` confound file generated by fMRIprep. It is also possible to specify a list of confound (or imaging) files, in which case `load_confounds` will return a list of numpy ndarray and the associated sample mask.
...
`load_confounds` supports flexible self-defined strategies. One can use the `Confounds` class to recreate a subset of strategies from Ciric et al. 2017. The following table highlights the relevant options:
